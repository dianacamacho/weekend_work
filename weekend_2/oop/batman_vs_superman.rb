...
class Superhero
  ...
  attr_reader :name, :has_special_tool, :initial_attack
  attr_accessor :alive, :hitpoints, :attack

  def initialize(hash)
    @name = hash[:name]
    @hitpoints = hash[:hitpoints]
    @attack = hash[:attack]
    # @initial_attack = hash[:attack]
    @alive = true
  ...
  def hit(enemy)
    ...
    attack_hitter = @attack

    p self # to double check state of each superhero

    enemy.hit_stats(attack_hitter)

  end

  def hit_stats(attack_hitter)
     
    @hitpoints = @hitpoints - attack_hitter 
